...
# Sem utilizar a função criada na tarefa anterior
...
answer = "Pois a coluna gênero não está completamente preenchida, ou seja, há linhas em que este campo esta em branco, diferenciando a soma male+female do total de linhas."
...
def trip_limits(data: List, min_time: float, max_time: float) -> Tuple[float, float]:
    ...
    :param min_time: tempo minimo de inicio
    :param max_time: tempo maximo de inicio
    :return: retorna a duração minima e maxima de viagem
    """
    for time in data:
        if min_time == 0:
            min_time = float(time)
        elif float(time) < min_time:
            min_time = float(time)
        if float(time) > max_time:
            max_time = float(time)
...
min_trip, max_trip = trip_limits(float_trip_duration_list, min_trip, max_trip)
mean_trip = trip_mean(float_trip_duration_list)
median_trip = trip_median(float_trip_duration_list)
...
def count_items(column_list):
    """
    Conta a quantidade de qualquer tipo de dado sem a necessidade de definições
